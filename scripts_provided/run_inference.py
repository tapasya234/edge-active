...
DLC_PATH = "/Users/tapasyagutta/workspace/edge-active/export_assets/job_j57d1jql5_optimized_bin_mm58d8okm.bin"  # e.g. "./export_assets/resnet_2plus1d.bin"
...
if not data_path:
    _errors.append(
        "'data_path' is not set. Point it to the directory of preprocessed .npy tensors."
    )
...
if USE_SINGLE_TENSOR and not SINGLE_TENSOR_PATH and not data_path:
    _errors.append(
        "USE_SINGLE_TENSOR is True but neither SINGLE_TENSOR_PATH nor data_path is set."
    )
...
def inference_job(model, device, dataset):
    job = qai_hub.submit_inference_job(
        model=model, device=device, inputs=dataset, options=""
    )
    return job.job_id
...
def _enforce_frames(x: np.ndarray, target_t: int) -> np.ndarray:
    ...
    if current_t < target_t:
        ...
        x = np.pad(
            x, ((0, 0), (0, 0), (0, target_t - current_t), (0, 0), (0, 0)), mode="edge"
        )
...
if USE_SINGLE_TENSOR:
    video_tensors = [
        load_single_tensor(data_path, SINGLE_TENSOR_PATH, SINGLE_TENSOR_INDEX)
    ]
else:
...
if DLC_PATH:
    ...
    input_specs = {"video": ((BATCH, C, T, H, W), "float32")}
